...
function encuentraPalabraMasLarga(cad_arg) {
    ...
    document.write("Palabra más larga encontrada: " + longitudMaxima + " caracteres");
}
...
function fltraPalabrasMasLargas(cad_arg, i) {
    ...
    document.write("La palabra más larga es: " + longitudMaxima);
}
...
function cadenaBienFormada(cad_arg) {
    ...
    document.write("Primera mayúscula y resto minúscula: " + primeraLetra + restoFrase);
}
...
function ConsonantesVocales(cadena) {
    ...
    for (let i = 0; i < noEspacios.length; i++) {  // Bucle hasta el tamaño de la cadena
        let letra = noEspacios.charAt(i); s
...
function palindromo(palabra) {
    ...
    document.write("Palabra al revés: " + alreves + "<br><br>")
...
function numeroPalabras(cadena) {
    ...
    document.write("Hay número de " + palabras.length + " palabras");
}
...
function validateCreditCard(numTarjeta) {
    ...
    if (numTarjetaString.length !== 16) {
        ...
        console.log("Tu tarjeta: " + numTarjeta + " no tiene 16 dígitos, vuelve a intentarlo.");
    } else {
        if (numDistintos.length < 2) {
            ...
            if (esPar(numTarjeta) !== true) {
                ...
                console.log("Tu tarjeta: " + numTarjeta + " tu último número no es par. Vuelve a intentarlo.");
            } else {
                if (sumaTodosNumeros <= 16) {
                    // Verificamos si la suma de los dígitos es mayor a 16
                    console.log("Tu tarjeta: " + numTarjeta + " la suma de todos los dígitos no es más de 16, vuelve a intentarlo.");
                } else {
                    validacion = true; // Marcamos la tarjeta como válida
                    console.log("Tu tarjeta " + numTarjeta + " es válida.");
                }
            }
        }
    }
...
function validateCreditCard2(numTarjeta) {
    ...
    // Sumar todos los dígitos de la tarjeta
    for (let i = 0; i < numTarjetaString.length; i++) {
        sumaTodosNumeros += parseInt(numTarjetaString.charAt(i)); // Sumar cada dígito
    }
