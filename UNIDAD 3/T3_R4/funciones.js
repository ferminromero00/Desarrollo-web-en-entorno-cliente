...
const saddlePoints = () => {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        // Recolectamos todos los valores de la fila i
        for (let e = 0; e < 5; e++) {
            arrayTemporal.push(mapa[i][e]);
        }

        // Encontramos el valor máximo de la fila y lo agregamos a filaArray
        const maxValor = Math.max(...arrayTemporal);
        filaArray.push(maxValor);
    }

    // Imprimimos los valores máximos de cada fila
    document.write(filaArray.map(valor => valor + "<br>"));
...
const codificarMensaje = (mensaje) => {
    ...
    document.write(resultados); // Muestra el mapa codificado
}
